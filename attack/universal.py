...
from attack.model_universal import *
...
def universal_train(inputs, blocks, args, load_path, sum_path='./output/universalattacking'):
# build up a new graph, and create a session as: sess
    ...
    # Define model loss
    train_loss, pred = build_noise_model(x, n_his, Ks, Kt, blocks, keep_prob, theta, batch_size, n, noise_para)
    ...
    universal_noise_var_list = [t for t in trainable_vars if t.name.startswith(u'unoise')]
...
def universal_test(inputs, batch_size, n_his, n_pred, inf_mode, load_path='./output/models/'):
    '''
    ...
    adversary = get_noise_universal()
    adversary = np.array(adversary[0])
    ...
    with tf.Session(graph=test_graph) as test_sess:
        ...
        x_test[:,0:n_his,:,:] = x_test[:,0:n_his,:,:] + np.tile(adversary,[np.size(x_test,0),1,1,1])
        ...
        #===================================save=========================
#        np.save('./output/ypred/ytest_universal_002',y_test)
#        np.save('./output/ypred/xtest_universal_002',x_test)
        #===================================save=========================
        for ix in tmp_idx:
            te = evl[ix - 2:ix + 1]
...
def get_noise_universal(load_path = './output/universalattacking/models/'):
    ...
    with tf.Session(graph=test_graph) as test_sess:
        ...
        unoise = test_graph.get_collection('unoise')
        adversary = test_sess.run(unoise)
    return adversary
